...
public unsafe class P3_Blackhole_Lite_SelfMark : SplatoonScript
{
    public override Metadata Metadata { get; } = new(11, "NightmareXIV, self-marker edit by ttolgang");
    public override HashSet<uint>? ValidTerritories { get; } = [1363];

    private ImGuiEx.RealtimeDragDrop[] DragDrop = [
        new("CarDir0", x => x.ToString()),
        new("CarDir1", x => x.ToString()),
        new("CarDir2", x => x.ToString())
    ];

    private ImGuiEx.RealtimeDragDrop[] DragDrop2 = [
        new("CarDir3", x => x.ToString()),
        new("CarDir4", x => x.ToString()),
        new("CarDir5", x => x.ToString())
    ];

    private uint Sequence = 0;
    private bool? IsAccretion = null;

    // Self-marker state. This is intentionally one-shot per reset/combat.
    private bool _selfMarkerScheduled = false;
    private bool _selfMarkerDone = false;
    private long _selfMarkerDueTick = 0;
    private string _pendingSelfMarkerCommand = "";
    private int _selfMarkerNumber = 0;
    ...
    public override void OnReset()
    {
        Sequence = 0;
        IsAccretion = null;
        ResetSelfMarkerState();
    }

    private void ResetSelfMarkerState()
    {
        _selfMarkerScheduled = false;
        _selfMarkerDone = false;
        _selfMarkerDueTick = 0;
        _pendingSelfMarkerCommand = "";
        _selfMarkerNumber = 0;
    }
    ...
    public override void OnUpdate()
    {
        ...
        TryScheduleSelfMarker();
        TryExecuteSelfMarker();
        ...
        if(valid)
        {
            if(myHoles.Count == 1)
            {
                ...
                if(C.ShowTether == ShowTether.Counter_Clockwise && Controller.TryGetElementByName("HoleCCW", out var e2))
                {
                    e2.Enabled = true;
                    e2.refActorObjectID = myHoles[0].ObjectId;
                }
            }
    ...
    private void TryScheduleSelfMarker()
    {
        if(!C.EnableSelfMarkerCommands) return;
        if(_selfMarkerDone || _selfMarkerScheduled) return;

        var ownNumber = GetOwnNumber();
        if(ownNumber == 0) return;

        var command = ownNumber switch
        {
            1 => C.Number1MarkerCommand,
            2 => C.Number2MarkerCommand,
            3 => C.Number3MarkerCommand,
            _ => ""
        };

        if(string.IsNullOrWhiteSpace(command)) return;

        var minDelay = Math.Clamp(C.SelfMarkerMinDelayMs, 0, 5000);
        var maxDelay = Math.Clamp(C.SelfMarkerMaxDelayMs, minDelay, 5000);
        var delay = Random.Shared.Next(minDelay, maxDelay + 1);

        _selfMarkerNumber = ownNumber;
        _pendingSelfMarkerCommand = command;
        _selfMarkerDueTick = Environment.TickCount64 + delay;
        _selfMarkerScheduled = true;
    }

    private void TryExecuteSelfMarker()
    {
        if(!_selfMarkerScheduled || _selfMarkerDone) return;
        if(Environment.TickCount64 < _selfMarkerDueTick) return;

        ExecuteSelfMarkerCommand(_pendingSelfMarkerCommand);

        _selfMarkerDone = true;
        _selfMarkerScheduled = false;
        _pendingSelfMarkerCommand = "";
    }

    private void ExecuteSelfMarkerCommand(string command)
    {
        if(string.IsNullOrWhiteSpace(command)) return;

        if(C.ExecuteSelfMarkerCommandsInLiveDuty)
        {
            Chat.Instance.SendMessage(command);
        }
        else
        {
            Svc.Chat.Print($"[P3 Blackhole Lite] Self-marker dry run: number {_selfMarkerNumber}, command '{command}'");
        }
    }
                ...
                if(seq != null && GetMyRole() != Taker.None)
                {
                    for(var i = 0; i < tethers.Count; i++)
                    {
                        ...
                        if(current == GetMyRole())
                        {
                            ...
                            if(!DrawHole(ref numElements, myHole.Value.Object, positions)) valid = false;
                            myHoles.Add(myHole.Value.Object);
                            isTaker = true;
                        }
                    }
                }
                if(!isTaker && tethers.Count > 0) DrawIdle();
            }
        }
        else
        {
                    ...
                    for(var i = 0; i < seq.Count; i++)
                    {
                        ...
                        foreach(var dir in C.TetherPriorities[i])
                        {
                            if(!takenDirection.Contains(dir) && tethers.TryGetValue(dir, out var blackhole))
                            {
                                ...
                                {
                                    if(!DrawHole(ref numElements, blackhole, positions)) valid = false;
    ...
    // Action #47868 Nothingness cast on DataId 19512 / Black Hole.
    ...
    private void DrawIdle()
    {
        ...
        if(Controller.TryGetElementByName("Idle", out var i))
        {
            i.Enabled = true;
            i.color = Controller.AttentionColor;
        }
    }
    ...
    public override void OnSettingsDraw()
    {
        ...
        {
            ImGuiEx.TextWrapped("Configure which marker takes which tether. If specified markers will not be present, the script will fail.");
        }
        ...
        if(ImGui.CollapsingHeader("Debug"))
        {
            ...
            ImGuiEx.Text($"Self marker scheduled: {_selfMarkerScheduled}");
            ImGuiEx.Text($"Self marker done: {_selfMarkerDone}");
            ImGuiEx.Text($"Self marker number: {_selfMarkerNumber}");
            ImGuiEx.Text("Marker:");
    ...
    private void DrawSelfMarkerSettings()
    {
        if(ImGui.CollapsingHeader("Self marker commands"))
        {
            ImGui.Checkbox("Enable self-marker commands", ref C.EnableSelfMarkerCommands);
            ImGui.Checkbox("Execute marker commands in live duty", ref C.ExecuteSelfMarkerCommandsInLiveDuty);
            ImGuiEx.TextWrapped("Off: print the command locally instead of sending it. On: actually send /mk command to yourself. Test with this OFF first.");

            ImGui.SetNextItemWidth(260f);
            ImGui.InputText("Number 1 command", ref C.Number1MarkerCommand, 100);
            ImGui.SetNextItemWidth(260f);
            ImGui.InputText("Number 2 command", ref C.Number2MarkerCommand, 100);
            ImGui.SetNextItemWidth(260f);
            ImGui.InputText("Number 3 command", ref C.Number3MarkerCommand, 100);

            ImGuiEx.TextWrapped("Commands are sent once when your Number 1/2/3 debuff is detected. Delay is useful if your strat needs marker-order control.");
            ImGui.SetNextItemWidth(120f);
            ImGui.InputInt("Min delay, ms", ref C.SelfMarkerMinDelayMs);
            ImGui.SetNextItemWidth(120f);
            ImGui.InputInt("Max delay, ms", ref C.SelfMarkerMaxDelayMs);
            C.SelfMarkerMinDelayMs = Math.Clamp(C.SelfMarkerMinDelayMs, 0, 5000);
            C.SelfMarkerMaxDelayMs = Math.Clamp(C.SelfMarkerMaxDelayMs, C.SelfMarkerMinDelayMs, 5000);

            if(ImGui.Button("Reset self-marker one-shot state"))
            {
                ResetSelfMarkerState();
            }
        }
    ...
    private bool DrawMkSel(Marker mk, ref uint currentValue)
    {
        ...
        {
            if(ImGui.ImageButton(tex.Handle, new(40))) ImGui.OpenPopup("MrkSlkt");
    ...
    public OrderedDictionary<CardinalDirection, (IBattleNpc Object, float AngleDegrees)> GetTetheredOrderedBlackholes()
    {
        var bh = GetTetheredBlackholes();
        if(bh.Count == 0 || Gigakefka == null) return [];

        var ret = new OrderedDictionary<CardinalDirection, (IBattleNpc Object, float AngleDegrees)>();
    ...
    public class Config
    {
        ...
        public bool UseKefkaRelative = true;
        ...
        // Added: self-mark command settings.
        public bool EnableSelfMarkerCommands = false;
        public bool ExecuteSelfMarkerCommandsInLiveDuty = false;
        public string Number1MarkerCommand = "/mk Attack <me>";
        public string Number2MarkerCommand = "/mk Bind <me>";
        public string Number3MarkerCommand = "/mk Ignore <me>";
        public int SelfMarkerMinDelayMs = 0;
        public int SelfMarkerMaxDelayMs = 0;
    }
